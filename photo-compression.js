(() => {
  ...
  const looksLikeImage = file => {
    ...
    // Android WebView / OEM camera can expose a valid content:// image with an empty MIME type.
  ...
  // Wrap the existing, proven photo-analysis function instead of intercepting the
  // input change event. This keeps the core handler, consent, monetization and UI
  // flow intact and guarantees a single upload attempt.
  ...
  // Expose the same wrapped function for extensions that access it through window.
