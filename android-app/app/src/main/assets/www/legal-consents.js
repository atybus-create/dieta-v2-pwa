(() => {
  ...
  async function ensureCurrentTerms() {
    ...
    try { const status = await refreshConsents(); if (status.termsAccepted) return true; } catch (error) { console.warn('Nie udało się sprawdzić Regulaminu:', error); }
    ...
    termsPromise = new Promise(resolve => {
      ...
      check.checked = false; button.disabled = true; error.classList.add('hidden'); overlay.classList.remove('hidden');
      button.onclick = async () => {
        ...
        try {
          ...
          consentState.termsAccepted = true; overlay.classList.add('hidden'); resolve(true); termsPromise = null;
        } catch (e) {
          error.textContent = e?.message || 'Nie udało się zapisać akceptacji Regulaminu.'; error.classList.remove('hidden'); button.disabled = false;
        } finally { button.textContent = 'Akceptuję i przechodzę dalej'; }
      };
    });
    return termsPromise;
  }
  ...
  function aiModal() {
    ...
    if (overlay) return overlay;
    overlay = document.createElement('div'); overlay.id = 'aiConsentModal'; overlay.className = 'legal-overlay hidden';
    ...
    document.body.appendChild(overlay);
    const check = document.getElementById('aiConsentCheck'); const button = document.getElementById('aiConsentAccept'); check.onchange = () => { button.disabled = !check.checked; };
    return overlay;
  }

  async function ensureAiConsent() {
    if (!state?.token) throw new Error('Brak aktywnej sesji.');
    if (!consentState.loaded) { try { await refreshConsents(); } catch (_) {} }
    if (consentState.aiConsentAccepted) return true;
    if (aiPromise) return aiPromise;
    aiPromise = new Promise(resolve => {
      const overlay = aiModal(); const check = document.getElementById('aiConsentCheck'); const button = document.getElementById('aiConsentAccept'); const cancel = document.getElementById('aiConsentCancel'); const error = document.getElementById('aiConsentError');
      check.checked = false; button.disabled = true; error.classList.add('hidden'); overlay.classList.remove('hidden');
      cancel.onclick = () => { overlay.classList.add('hidden'); aiPromise = null; resolve(false); };
      button.onclick = async () => {
        if (!check.checked) return;
        button.disabled = true; button.textContent = 'Zapisuję…';
        try {
          await consentRequest('consent_accept', { consentType: 'ai_processing', consentVersion: AI_CONSENT_VERSION, accepted: true });
          consentState.aiConsentAccepted = true; overlay.classList.add('hidden'); updateLegalPanel(); aiPromise = null; resolve(true);
        } catch (e) {
          error.textContent = e?.message || 'Nie udało się zapisać zgody.'; error.classList.remove('hidden'); button.disabled = false;
        } finally { button.textContent = 'Zgadzam się i rozpocznij analizę'; }
      };
    });
    return aiPromise;
  }
  ...
  function ensureRegistrationTerms() {
    ...
    const create = document.getElementById('createUserBtn'); if (!create) return;
  ...
  function updateLegalPanel() {
    ...
    if (badge) { badge.textContent = consentState.aiConsentAccepted ? 'Aktywna' : 'Nieaktywna'; badge.classList.toggle('off', !consentState.aiConsentAccepted); }
    const toggle = document.getElementById('aiConsentManage'); if (toggle) toggle.textContent = consentState.aiConsentAccepted ? 'Wycofaj zgodę AI' : 'Włącz analizę AI';
  }

  function ensureSettingsPanel() {
    if (document.getElementById('legalPrivacyPanel')) return;
    const theme = document.getElementById('themePanel'); if (!theme) return;
    const panel = document.createElement('div'); panel.id = 'legalPrivacyPanel'; panel.className = 'panel glass-card legal-panel';
    ...
    document.getElementById('aiConsentManage').onclick = async () => {
      if (consentState.aiConsentAccepted) {
        ...
        try { await consentRequest('consent_withdraw', { consentType: 'ai_processing', consentVersion: AI_CONSENT_VERSION, accepted: false }); consentState.aiConsentAccepted = false; updateLegalPanel(); toast('Zgoda na analizę AI została wycofana.'); } catch (e) { toast(e?.message || 'Nie udało się wycofać zgody.'); }
      } else { await ensureAiConsent(); }
    };
    updateLegalPanel();
  }
  ...
  function ensureEstimateNote() {
    ...
    const save = document.getElementById('saveMealBtn'); if (!save) return;
    const note = document.createElement('div'); note.id = 'aiEstimateNote'; note.className = 'ai-estimate-note'; note.textContent = 'Analiza AI jest szacunkiem. Sprawdź składniki i ilości przed zapisaniem.'; save.insertAdjacentElement('beforebegin', note);
  }
  ...
  createUser = async function legalAwareCreateUser(...args) {
    ...
    if (!checked) { setAuthError('Aby utworzyć profil, zaakceptuj Regulamin aplikacji.'); document.getElementById('registrationTermsCheck')?.focus(); return; }
    const originalPost = post;
    post = async function legalCreatePost(url, payload = {}, file = null) {
      if (url === API && payload?.action === 'user_create') payload = { ...payload, termsAccepted: true, termsVersion: TERMS_VERSION, appVersion: APP_VERSION };
      return originalPost(url, payload, file);
    };
    try { return await baseCreateUser(...args); } finally { post = originalPost; }
  };

  const baseApi = api;
  api = async function legalAwareApi(action, payload = {}, file = null) {
    const normalized = String(action || '').trim().toLowerCase();
    if (AI_ACTIONS.has(normalized)) { const allowed = await ensureAiConsent(); if (!allowed) throw new Error('Analiza AI nie została uruchomiona.'); }
    return baseApi(action, payload, file);
  };

  const baseEnterApp = enterApp;
  enterApp = async function legalAwareEnterApp(...args) {
    const accepted = await ensureCurrentTerms(); if (!accepted) return;
    const result = await baseEnterApp(...args); ensureSettingsPanel(); ensureEstimateNote(); try { await refreshConsents(); } catch (_) {} return result;
  };

  const baseClearSession = clearSession;
  clearSession = function legalAwareClearSession(...args) { consentState = { loaded: false, termsAccepted: false, aiConsentAccepted: false }; termsPromise = null; aiPromise = null; return baseClearSession(...args); };

  function initLegal() { injectStyles(); ensureRegistrationTerms(); ensureSettingsPanel(); ensureEstimateNote(); }
